...
- (void)setupInitialProperty
{
    ...
    NSString *defaultDateString = @"15-07-1990";
...
- (void)selectDefaultDateString:(NSString *)defaultDateString
{
    if (!defaultDateString || [defaultDateString isEqualToString:@""]) {
        defaultDateString = @"15-07-1990";
    }
    
    NSArray *listItems = [defaultDateString componentsSeparatedByString:@"-"];

    
    NSInteger year = [listItems[2] integerValue];
    NSInteger month = [listItems[1] integerValue];
    NSInteger day = [listItems[0] integerValue];
...
- (void)pickerView:(UIPickerView *)pickerView didSelectRow:(NSInteger)row inComponent:(NSInteger)component
{
    ...
    for (NSInteger i = 0; i < dayCount; i++) {
        [dayArray addObject:[NSString stringWithFormat:@"%ld",(long)i + 1]];
    }
...
- (NSString *)selectDateString
{
    ...
    NSString *dateString = [NSString stringWithFormat:@"%lu-%02lu-%02lu",
                            (unsigned long)selectDayRow + 1, (unsigned long)selectMonthRow + 1,
                            (unsigned long)selectYearRow + self.beginYear];
    return dateString;
}
...
- (NSMutableArray *)yearArray
{
    if (!_yearArray) {
        ...
        {
            [_yearArray addObject:[NSString stringWithFormat:@"%lu",(unsigned long)i]];
        }
    }
    return _yearArray;
}

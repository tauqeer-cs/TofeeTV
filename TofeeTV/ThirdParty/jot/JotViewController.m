...
- (instancetype)initWithImage:(UIImage *)image parentView:(UIView *)parentView
{
    if ((self = [super init])) {
        ...
        self.backgroundImageToUse = image;
        
        if (image) {
            
            [_drawView setFrame:CGRectMake(0, 0, image.size.width, image.size.height)];
        ...
        if (self.backgroundImageToUse) {
            
       
            int centerX = parentView.center.x - image.size.width/2;
            int centerY = parentView.center.y - image.size.height/2;
            
            [_drawingContainer setFrame:CGRectMake(centerX,
                                                   centerY,
                                                   image.size.width,
                                                   image.size.height)];
            
            [_drawingContainer setBackgroundColor:[UIColor lightGrayColor]];
            
            UIImageView *newImage=[[UIImageView alloc]initWithFrame:CGRectMake(0,
                                                                               0,
                                                                               image.size.width,
                                                                               image.size.height)];
            
            if (self.dontAddImage) {
                newImage=[[UIImageView alloc]initWithFrame:CGRectMake(0,
                                                                      0,
                ...
                [self.view setBackgroundColor:[UIColor grayColor]];
                       [_drawingContainer setBackgroundColor:[UIColor grayColor]];
            }
            
            [newImage setImage:self.backgroundImageToUse];
            
            [_drawingContainer addSubview:newImage];
            [_drawingContainer sendSubviewToBack:newImage];
...
{
    [super viewDidLoad];
    //here else you can add image


    AppDelegate *appDelegate =  (AppDelegate *)[UIApplication sharedApplication].delegate;
   // appDelegate.usingPan = YES;
...
- (UIImage *)renderImageWithSize:(CGSize)size onColor:(UIColor *)color
{
    ...
    if (self.backgroundImageToUse) {
        if (self.dontAddImage) {
            ...
            colorImage = self.backgroundImageToUse;

            UIImage *tmp =colorImage;
            
            tmp = [tmp imageByApplyingAlpha:0.0];
            colorImage = tmp;
